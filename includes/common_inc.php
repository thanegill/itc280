 ...
 *
 * Version 2.10 changed htmlSelector() to selector(), changed get/showFlash() to feedback()/showFeedback()
 * 
 * @package nmCommon
...
function formReq($var) {
	...
	if(!isset($_POST[$var]))
	{
		feedback("Required Form Data Not Passed");
...
function get_footer($path = '') {
	...
	if(isset($config->benchmarking) && $config->benchmarking == true && $headerDone == 2) {
		...
		if(startSession() && isset($_SESSION['AdminID'])) {
			$config->copyright .= ' <em>(Page load: ' . $mytime . ' seconds.)</em> ';
		}
	}
 ...
 *
 * <code>
 * feedback('Flash!  This is an important message!'); #will show up next running of showFlash()
 * </code>
 * 
 * added version 2.07
 *
 ...
 * </code>
 * 
 * changed from showFlash() version 2.10
 *
 * @param none 
...
function showFeedback() {
	...
	if(isset($_SESSION['feedback']) && $_SESSION['feedback'] != "") {
		...
		$myReturn .= '<div class="feedback ' . $level . '">' . $level . ": "  . $_SESSION['feedback'] . '</div>';
