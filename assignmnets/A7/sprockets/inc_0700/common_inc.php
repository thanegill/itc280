 ...
 *
 * Version 2.07 adds htmlSelector(), smartTitle() addLink() and get/showFlash() functions
...
function get_header($myInclude = '')
{
	...
	if($config->benchmarking && $headerDone == 2)
...
function rte($RTEID,$Width='100%',$Height='400',$ToolBar = 'Basic',$showBorder=FALSE)
{
	...
	{# only admins can see edit fields
		echo '<div align="center"><form name="exitForm" action="' . ADMIN_DASHBOARD . '" method="get">';
 ...
 * htmlSelector("sidebar2",$html,"muffin_list.php~muffin_view.php","file","replace","~");
 * </code>
 *
 ...
 * htmlSelector("sidebar2",$html,"model_","string","after","~");
 * </code>
 *
 ...
 * @param string $selector "file" or "string", identifies what we're trying to match
...
function htmlSelector($property="",$html="",$match="",$selector="file",$location="replace",$separator="~")
{
	global $config;
	$selector = strtolower($selector); #de-specify case
	$foundMatch = FALSE; #will turn to true if match
	if(strtolower($selector) == "string")
...
#load it here and show it one time when showFlash() is called

/**
 * loads a quick user message (flash/heads up) to provide user feedback
 *
 * Uses a Session to store the data until the data is displayed via showFlash() 
 *
 * <code>
 * loadFlash('Flash!  This is an important message!'); #will show up next running of showFlash()
 * </code>
 * 
 * added version 2.07
 *
 * @param string $msg message to show next time showFlash() is invoked
 * @return none 
 * @see showFlash() 
 * @todo none
 */

function loadFlash($msg)
{
	startSession();
	$_SESSION['flash'] = $msg;
}#end loadFlash();
 ...
 * Uses a Session to store the data until the data is displayed via showFlash()
 *
 * Related loadFlash() function used to store message 
 *
 * <code>
 * showFlash(); #will show then clear message stored via loadFlash()
 * </code>
 * 
 * added version 2.07
 *
 * @param none 
 * @return none outputs via internal echo
 * @see loadFlash() 
 * @todo none
 */
function showFlash()
{
	if(startSession() && isset($_SESSION['flash']) && $_SESSION['flash'] != "")
	{#show message, clear flash
		echo '<div align="center"><h3><font color="red">' . $_SESSION['flash'] . '</font></h3></div>';
		$_SESSION['flash'] = ""; #cleared
	}
}#end showFlash()
