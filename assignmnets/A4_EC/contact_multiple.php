<?php
/**
 * contact_multiple.php is a postback virtually identical to contact.php with the 
 * exception that it demonstrates how to add radio buttons, checkboxes & selects to
 * the form.
 *
...
<form class="comment_form contact_wide" action="<?php echo basename($_SERVER['PHP_SELF']); ?>" method="post" onsubmit="return checkForm(this);">
<?php
...
if (!isset($_POST["recaptcha_response_field"])|| $error != "") {
	...
	<h1>Contact Form</h1>
	<h3>Assignmnet 4 Extra Credit</h3>
	<table>
		<!-- the form elements 'Name' and 'Email' are sigificant to the app, any others can be added/deleted -->
		<tr>
		...
		<tr>
			...
			<td>
				<select name="How_Did_You_Hear_About_Us?">
					...
					<option value="A Friend">Class</option>
					<option value="A Friend">Ad</option>
					<option value="Other">Other</option>
				</select>
			</td>
		</tr>
		<tr>
			<td align="right">What music are you intersted in?</td>
			<td>
				<input type="checkbox" name="Music[]" value="Contry"/> Contry<br />
				<input type="checkbox" name="Music[]" value="Rap"/> Rap<br />
				<input type="checkbox" name="Music[]" value="Hip-Hop"/> Hip-Hop<br />
				<input type="checkbox" name="Music[]" value="Electonic"/> Electonic<br />
		...
		<tr>
			<td align="right">Do you like pie?</td>
			<td>
		...
		<tr>
			<td class="text"><p>Other Comment:</p></td>
...
//include 'include/header_inc.php'; //PLACE REFERENCE TO YOUR FOOTER FILE HERE
